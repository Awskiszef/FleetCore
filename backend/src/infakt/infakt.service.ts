...
export class InfaktService {
  ...
  private async getHeaders() {
    const setting = await this.prisma.setting.findUnique({ where: { key: 'infaktApiKey' } });
    const apiKey = setting?.value || process.env.INFAKT_API_KEY;

    if (!apiKey || apiKey === 'wprowadz_tutaj_swoj_klucz_api_infakt') {
      throw new HttpException('Brak ważnego klucza API inFakt w ustawieniach lub pliku .env', HttpStatus.INTERNAL_SERVER_ERROR);
    }
    return {
  ...
  async findOrCreateClient(customerData: any): Promise<number> {
    ...
    if (customerData.address) {
      ...
      const zipMatch = customerData.address.match(/((?:\d{2}-\d{3}|\d{3}\s\d{2}|\d{4,5}))\s+([^,]+)/);
      if (zipMatch) {
        parsedPostalCode = zipMatch[1];
        parsedCity = zipMatch[2].trim();
        
        let remaining = customerData.address.replace(zipMatch[0], '').trim();
        remaining = remaining.replace(/,\s*,\s*/g, ', ').replace(/,\s*$/, '').trim();
        ...
        if (parts.length > 1) {
          ...
          parsedStreet = parts.slice(0, parts.length - 1).join(',').trim();
        } else {
    ...
    if (customerData.nip) {
      ...
      try {
        const response = await fetch(`${this.apiUrl}/clients.json?q[company_name_or_nip_eq]=${customerData.nip}`, {
    ...
    if (clientId) {
      ...
      if (!response.ok) {
        ...
        throw new HttpException(`Błąd podczas tworzenia klienta w inFakt: ${JSON.stringify(err)}`, HttpStatus.BAD_REQUEST);
      }
    ...
    if (!clientId) {
      throw new HttpException('Nie udało się uzyskać ID klienta z inFakt', HttpStatus.INTERNAL_SERVER_ERROR);
    }
  ...
  // 2. Wystaw fakturę
  async createInvoice(orderData: any, clientId: number): Promise<{ id: number, uuid: string }> {
    const today = new Date().toISOString().split('T')[0];
    const vehicleInfo = orderData.vehicle ? `${orderData.vehicle.make} ${orderData.vehicle.model} rej. ${orderData.vehicle.licensePlate}` : 'Pojazd nieznany';
    ...
    if (!response.ok) {
      const err = await response.json().catch(() => ({}));
      throw new HttpException(`Błąd podczas wystawiania faktury w inFakt: ${JSON.stringify(err)}`, HttpStatus.BAD_REQUEST);
    }
  ...
  async updateInvoiceStatus(invoiceId: string, status: string): Promise<void> {
    ...
    if (!response.ok) {
      const err = await response.json().catch(() => ({}));
      throw new HttpException(`Błąd podczas zmiany statusu faktury: ${JSON.stringify(err)}`, HttpStatus.BAD_REQUEST);
    }
  }
  ...
  async downloadInvoicePdf(invoiceId: string): Promise<ArrayBuffer> {
    const response = await fetch(`${this.apiUrl}/invoices/${invoiceId}/pdf.json?document_type=original`, {
      headers: await this.getHeaders(),
    });
    if (!response.ok) {
      throw new HttpException('Nie można pobrać pliku PDF z inFakt', HttpStatus.NOT_FOUND);
    }
  ...
  async getInvoicesByMonth(year: number, month: number): Promise<any[]> {
    ...
    const response = await fetch(`${this.apiUrl}/invoices.json?q[invoice_date_gteq]=${startDate}&q[invoice_date_lteq]=${endDate}&limit=100`, {
      headers: await this.getHeaders(),
    });

    if (!response.ok) {
      throw new HttpException('Nie można pobrać faktur z inFakt', HttpStatus.INTERNAL_SERVER_ERROR);
    }
