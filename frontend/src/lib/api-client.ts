...
export const apiClient = {
  async getCustomers(params?: PaginationParams): Promise<PaginatedResponse<unknown>> {
    const res = await fetch(buildUrl('/customers', params));
    if (!res.ok) throw new Error('Failed to fetch customers');
    return res.json();
  },
  
  async getVehicles(params?: PaginationParams): Promise<PaginatedResponse<unknown>> {
    const res = await fetch(buildUrl('/vehicles', params));
    if (!res.ok) throw new Error('Failed to fetch vehicles');
    return res.json();
  },

  async getRepairOrders(params?: PaginationParams): Promise<PaginatedResponse<unknown>> {
    const res = await fetch(buildUrl('/repair-orders', params));
    if (!res.ok) throw new Error('Failed to fetch repair orders');
    return res.json();
  },

  async getParts(params?: PaginationParams): Promise<PaginatedResponse<unknown>> {
    const res = await fetch(buildUrl('/parts', params));
    if (!res.ok) throw new Error('Failed to fetch parts');
    return res.json();
  },

  async getUsers(params?: PaginationParams): Promise<PaginatedResponse<unknown>> {
    const res = await fetch(buildUrl('/users', params));
    if (!res.ok) throw new Error('Failed to fetch users');
    return res.json();
  },

  async getSuppliers(params?: PaginationParams): Promise<PaginatedResponse<unknown>> {
